...
{
  const configDir: string | undefined = Deno.env.get('HOME')
    ? join(Deno.env.get('HOME')!, 'Library', 'Application Support')
    : undefined;
  if (!configDir)
  {
...
Deno.test('initConfig (pr-maker) should load default pr-maker config and handle prompts', async () =>
{
  ...
  finally
  {
    ...
    if (originalOrg === undefined) Deno.env.delete('GITHUB_ORG');
    else Deno.env.set('GITHUB_ORG', originalOrg);
    if (originalRepo === undefined) Deno.env.delete('REPO_NAME');
    else Deno.env.set('REPO_NAME', originalRepo);
    if (originalLabels === undefined) Deno.env.delete('PR_LABELS');
    else Deno.env.set('PR_LABELS', originalLabels);
